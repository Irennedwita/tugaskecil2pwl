...
const dropdownElement = document.querySelector("dropdownMenu form-control");
const formElement = document.querySelector("searchForm input-group mb-3");
const searchInputElement = docuemnt.querySelector("searchInput form-control");
...
const searchPetByKey = (key) => {
  const filteredPet = petList.filter((pet) => {
    return pet.name.toLowerCase().includes(key.toLowerCase());
  });
